...
export default function AppNav() {
  ...
  if (isAdmin) {
    return (
      <nav className="sticky top-0 z-50 bg-[#070A12] border-b border-white/8">
    ...
    <>
      ...
      {authed && (
        <Link href="/activechallenge" className={`flex items-center gap-1.5 ${isActive("/activechallenge") ? activeLink : normalLink}`} onClick={() => setOpen(false)}>
          {hasActiveClaim && <span className="h-1.5 w-1.5 rounded-full bg-[#FF9B6A] animate-pulse" />}
          Verify
        </Link>
      )}
      {authed && (
  ...
  return (
    <nav className="sticky top-0 z-50 bg-[#070A12] border-b border-white/8" ref={menuRef}>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="sm:hidden absolute left-0 right-0 top-14 bg-[#0B0F1C] border-b border-white/10 py-4 px-6 flex flex-col gap-5 text-sm font-medium shadow-xl">
          {athleteLinks}
        </div>
      )}
    </nav>
