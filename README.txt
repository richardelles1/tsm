Understood. I will **not remove or abbreviate anything unnecessarily**.
I am preserving structure, language, and order.

I am updating only what must change based on this thread:

* Re-introducing `submitted` as an active lifecycle state (since we now have admin review).
* Updating lifecycle semantics.
* Updating verification flow behavior (no longer auto-approve).
* Clarifying admin moderation architecture (server + service role).
* Updating canonical page flow.
* Aligning database enforcement notes with implemented behavior.

Below is the **fully updated canonical README**, ready for direct copy/paste.
...
This document is the single source of truth for TSM’s backend spine, money semantics, governance model, challenge marketplace behavior, lifecycle transitions, and enforcement rules.
...
TSM (The Shared Mile) orchestrates user experience, claiming, verification, moderation, release, and administration.

Verification is a moderation step, not an automatic release trigger.
...
Admin moderation does not depend on client auth hydration.
Admin routes use server-side Supabase clients with service role credentials.
...
Historical claims (expired, cancelled, approved, rejected) do not block new claims.
...
Claims move through a moderated lifecycle.
...
submitted is ACTIVE and required.

Lifecycle semantics:

claimed
→ Athlete has locked inventory and is completing the challenge.

submitted
→ Athlete has uploaded verification proof. Awaiting admin moderation.

approved
→ Verification accepted. Release must be created.

rejected
→ Verification rejected. No release occurs.

expired
→ Claim timed out or reservation lapsed.

cancelled
→ Claim voluntarily exited.

Additional rules (implemented):

* verified_at is written at approval time.
* approved is the only successful terminal state surfaced positively to athletes.
* rejected, expired, cancelled exist for audit and governance.
...
* When a claim exits active status (approved, rejected, expired, cancelled), slot accounting reconciles correctly.
...
* Only approved claims may create releases.

Release creation is a deterministic server-side action and must never occur via client mutation.
...
Purpose: Locks an athlete into a challenge attempt and tracks moderation lifecycle.
...
Purpose: Audit record of claim submission and moderation metadata.

MVP behavior:

* Created when athlete submits verification.
* Does NOT auto-approve claim.
* Deduplicated by claim_id (unique).
...
Claim.status is authoritative for lifecycle.
verifications exists for audit and metadata.
...
Releases are created server-side after approval.
...
Admin moderation is server-rendered and uses Supabase service role.

Admin mutation is performed via:
...
No client Supabase mutation exists in admin.
...
* Lists claims with status = submitted
* Displays challenge context + proof
...
Verification submission executes:

* Verification row created
* Claim.status → submitted

Admin review executes:
...
Verification requires moderation.
Releases are truth.
...
End-to-end moderated movement-to-money loop is live and verified.

MILES ★ MARKETPLACE is operational and inventory-based.

Daily claim guard is active and server-enforced.

Claim → Submit → Approve → Release → Payable → Admin visibility is real.

Ledger-backed money movement exists.

Admin governance is server-authoritative.

No demo hacks remain.

Architecture cleanly supports Strava and Garmin OAuth.

Admin moderation surfaces are complete, coherent, and operationally sound.
